/**
 * AutoSales Agent — SVG Icon 图标库
 * 使用 lucide 风格图标
 */

const Icons = {
  brandMark: `<svg class="brand-mark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" aria-hidden="true"><rect width="64" height="64" rx="14" fill="#0f172a"/><path d="M13 38h6l5-10c1.1-2.1 2.8-3 5-3h11c2.1 0 3.8.9 5 2.8l5.9 10.2H56" fill="none" stroke="#f8fafc" stroke-width="4.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M25 29h15l4.8 9H20.5L25 29Z" fill="#2563eb"/><path d="M28 30h7v7h-10l3-7Z" fill="#93c5fd"/><path d="M37 30h4.2l3.6 7H37v-7Z" fill="#bfdbfe"/><circle cx="21" cy="42" r="5.5" fill="#f8fafc"/><circle cx="21" cy="42" r="2.4" fill="#0f172a"/><circle cx="48" cy="42" r="5.5" fill="#f8fafc"/><circle cx="48" cy="42" r="2.4" fill="#0f172a"/><rect x="45" y="15" width="9" height="9" rx="2" fill="#22c55e"/><path d="M49.5 12v3m0 9v3m-7.5-7.5h3m9 0h3" stroke="#86efac" stroke-width="1.8" stroke-linecap="round"/></svg>`,
  ...
  car: `<svg data-icon="garage" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 18V9l8-5 8 5v9"/><path d="M7 18v-6h10v6"/><path d="M8.5 15h7"/><path d="M9.5 12.5h5"/><circle cx="9" cy="18" r="1.4"/><circle cx="15" cy="18" r="1.4"/></svg>`,
...
/**
 * 生成侧边栏 HTML（在页面中插入）
 * @param {string} activePage - 当前活跃页面标识
 */
function renderSidebar(activePage) {
  const items = [
    { id: 'dashboard',    label: '概览',              icon: Icons.dashboard,  href: '/dashboard.html' },
    { id: 'chat',         label: 'AI 销售对话',        icon: Icons.chat,       href: '/chat.html' },
    { id: 'cars',         label: '车型库',              icon: Icons.car,        href: '/cars.html' },
...
function renderTopbar(pageTitle) {
  const modelBadge = window.__MODEL_MODE === 'deepseek'
    ? '<span class="topbar-badge deepseek" id="model-badge">DeepSeek</span>'
    : '<span class="topbar-badge mock" id="model-badge">Mock LLM</span>';
  ...
  return `
    <header class="topbar">
      ...
      <div class="topbar-right">
        <span class="topbar-badge online">
          <span style="width:6px;height:6px;border-radius:50%;background:currentColor;display:inline-block;"></span>
          广州天河体验店
        </span>
        ${modelBadge}
        <span style="color:var(--text-muted);font-size:12px;" id="topbar-time"></span>
      </div>
    </header>
  `;
}
...
function updateClock() {
  ...
  if (el) {
    const now = new Date();
    el.textContent = now.toLocaleString('zh-CN', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
  }
}
setInterval(updateClock, 1000);

/**
 * 页面骨架：在 body 中注入侧边栏+顶栏+内容区
 * @param {string} pageTitle - 页面标题
 * @param {string} activePage - 活跃导航标识
 */
    ...
    <div class="app-layout">
      ...
      <div class="main-wrapper">
        ...
        <main class="content" id="app-content">
          <div style="display:none;" id="app-content-inner"></div>
        </main>
      </div>
    </div>
  `;
